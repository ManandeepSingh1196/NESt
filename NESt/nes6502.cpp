...
nes6502::nes6502()
{
	//Creating the initialization table for the instruction sets, a has been used to make the instructions a bit shorter.
	using a = nes6502;
	lookup =
	{
...
void nes6502::clock()
{
	if (cycles == 0)
	{
		...
		uint8_t additional_cycle1 = (this->*lookup[opcode].addrmode)();				//Both the functions
																					//request additional cycles
		uint8_t additional_cycle2 = (this->*lookup[opcode].operate)();				//if page ends, so

		cycles += (additional_cycle1 & additional_cycle2);							//increase a cycle if both functions request
	}
...
//ZERO PAGE OFFSET WITH X REGISTER
uint8_t nes6502::ZPX()
{
	addr_abs = (read(pc) + x);
	pc++;
...
//ZERO PAGE OFFSET WITH Y REGISTER
uint8_t nes6502::ZPY()
{
	addr_abs = (read(pc) + y);
	pc++;
...
uint8_t nes6502::IND()
{
	...
	//There is a bug in the physical 6502, where if the low byte is at the end of the page (0XFF) and reads to the next,
	//it actuallly wraps back to the beginning of the page, rather than moving to the next.
	//This causes the return of an invalid address. To emulate the CPU, we need to emulate this bug
	
	if (low == 0x00FF)													//Simulating the bug
...
uint8_t nes6502::IZX()
{
	...
	uint16_t low = read((uint16_t)(ptr + (uint16_t)x) & 0x00FF);
	uint16_t high = read((uint16_t)(ptr + (uint16_t)x + 1) & 0x00FF);
...
uint8_t nes6502::RTI()
{
	...
	stkp++;
	pc |= (uint16_t)read(0x0100 + stkp) << 8;			// High and low reconstructed in one go
...
// A Non-Maskable Interrupt behaves in exactly the same way as a
// regular IRQ, but reads the new program counter address
